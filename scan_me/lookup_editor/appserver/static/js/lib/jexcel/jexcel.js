 ...
 */
 ...
 if (! window.jSuites && typeof(require) === 'function') {
    window.jSuites = require('../jsuites/jsuites');
    // I get an error with the following (https://lukemurphey.net/issues/2839)
    // require('window.jSuites/dist/window.jSuites.css');
}
    ...
    var jexcel = (function(el, options) {
        ...
        obj.prepareTable = function() {
            ...
            if (! multiple.length) {
                ...
                window.jSuites.ajax(multiple, function() {
                    obj.createTable();
                });
            }
        }
        ...
        obj.createTable = function() {
            ...
            obj.searchInput.placeholder = "Search Lookup"
            ...
            // Create element
            window.jSuites.contextmenu(obj.contextMenu, {
                onclick:function() {
...
console.log(ret);
        ...
        obj.parseValue = function(i, j, value) {
            ...
            if (obj.options.columns[i].mask) {
                ...
                value = '' + window.jSuites.mask.run(value, obj.options.columns[i].mask, decimal);
            }
        ...
        obj.createCell = function(i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    // Try formatted date
                    var formatted = window.jSuites.calendar.extractDateFromString(value, obj.options.columns[i].options.format);
                    // Create calendar cell
                    td.innerText = window.jSuites.calendar.getDateString(formatted ? formatted : value, obj.options.columns[i].options.format);
            ...
            for (var i = 0; i < toolbar.length; i++) {
                if (toolbar[i].type == 'i') {
                   ...
                   obj.toolbar.appendChild(toolbarItem);
                     ...
                     toolbarItem.innerText = toolbar[i].content;
                     window.jSuites.color(toolbarItem, {
        ...
        obj.openFilter = function(columnId) {
            if (! obj.options.filters) {
                ...
                // Dynamic dropdown
                window.jSuites.dropdown(div, opt);
            }
        }
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        window.jSuites.dropdown(editor, options);
                        ...
                        if (obj.options.columns[x].type == 'color') {
                            window.jSuites.color(editor, obj.options.columns[x].options);
                        } else {
                            window.jSuites.calendar(editor, obj.options.columns[x].options);
                        }
                        // Focus on editor
                        editor.focus();
                        ...
                        editor.appendChild(div);
                        window.jSuites.image(div, obj.options.imageOptions);
                        const rect = cell.getBoundingClientRect();
                        const rectContent = div.getBoundingClientRect();
                        if (window.innerHeight < rect.bottom + rectContent.height) {
                            div.style.top = (rect.top - (rectContent.height + 2)) + 'px';
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && ! force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        // Update calendar
                        var formatted = window.jSuites.calendar.extractDateFromString(value, obj.options.columns[x].options.format);
                        // Update data and cell
                        obj.options.data[y][x] = value;
                        obj.records[y][x].innerText = window.jSuites.calendar.getDateString(formatted ? formatted : value, obj.options.columns[x].options.format);
                    } else if (obj.options.columns[x].type == 'color') {
                        // Update color
                        obj.options.data[y][x] = value;
                        // Render
                        ...
                        if (obj.content.scrollTop + obj.content.clientHeight >= obj.content.scrollHeight) {
                            if (obj.loadDown()) {
                                if (obj.content.scrollTop + obj.content.clientHeight > obj.content.scrollHeight - 10) {
                                    let newScrollTop = obj.content.scrollTop - obj.content.clientHeight;
                                    // In jexcel LazyLoading implemented in a way that it will render 100 rows initially and when user scroll to bottom it will load next 30 rows and remove first 30 rows from table.
                                    // Based on that we have calculated these numbers:
                                    // 30(Row count) * 24(Row height) + 31(Header height) + 1 = 752
                                    if(newScrollTop <= 752){
                                        // 31 is header height. It was working with only obj.content.clientHeight but if we add 31 it gives little bit smooth experience.
                                        newScrollTop = obj.content.clientHeight + 31;
                                    }
                                    obj.content.scrollTop = newScrollTop;
                                }
                                obj.updateCornerPosition();
                            }
